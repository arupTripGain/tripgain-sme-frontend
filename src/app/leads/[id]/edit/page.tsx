      ...
      try {
        ...
        if (res.ok) {
          ...
          setFormData({
            ...
            companySize: data.organization?.employeeSize || '',
            companyPhone: data.organization?.phone || '',
            city: data.city || '',
            linkedinUrl: data.linkedinUrl || '',
            personalizedLine: data.personalizedLine || '',
            personalizationTrigger: data.personalizationTrigger || ''
          });
        }
      } catch (err) {
        console.error(err);
    ...
    <div className="flex h-screen flex-col w-full bg-[#fafafa]">
      ...
      <div className="flex items-center justify-between px-8 py-6 border-b border-border bg-card shrink-0">
        <div className="flex items-center gap-4">
          <Link href={`/leads/${id}`} className="text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="font-heading text-2xl font-bold text-secondary">Edit Lead</h1>
            <p className="text-sm text-muted-foreground mt-1">Update contact and organization information</p>
          </div>
        </div>
      </div>
